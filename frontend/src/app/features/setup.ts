...
@if(!window().length&&!busy()){<p>Selecciona rival y corte para ver los encuentros. No se incluyen partidos del día de corte.</p>}
...
export class Setup {
 api=inject(Api);router=inject(Router);teams=signal<Models['Team'][]>([]);targets=signal<Models['Match'][]>([]);window=signal<Models['Match'][]>([]);busy=signal(false);error=signal('');rival='Barcelona';analyst='';cutoff='2016-03-01';mode='date';target:number|null=null;
